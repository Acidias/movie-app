...
def analyze_subtitle2(file_path):
    with open(file_path, 'r') as file:
        text = file.read()
        blocks = re.findall(r'(\d{2}:\d{2}:\d{2},\d+) --> (\d{2}:\d{2}:\d{2},\d+)\n(.*?)\n\n', text, re.DOTALL)

        text_by_minute = defaultdict(str)

        for start, end, text in blocks:
            hour, minute, second, _ = map(int, re.split('[:,]', start))
            total_minutes = hour * 60 + minute  # converting hours to minutes and adding to the minute part
        ...
        for idx, (minute, text) in enumerate(sorted(text_by_minute.items())):
            response = openai.ChatCompletion.create(
                ...
                api_key='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'
            )
